...
class XtLoginRegisterHandler(private val serverContext: ServerContext) : MessageHandler<XtMessage> {
    ...
    override suspend fun handle(
        ...
        var status = verifyUser(connection.playerId, request)
        ...
        val payload = when (status) {
            ...
            // APPROVED
                ...
                if (result.isFailure) {
                    ...
                    val playerId = result.getOrThrow()
                    connection.playerId = playerId
                    // [userId, playerId]
                    arrayOf(playerId, playerId)
                }
            }
        }
     ...
     */
    private suspend fun verifyUser(playerId: Int, req: LreRequest): Int {
        ...
        // Otherwise -> approved, mark as online
        serverContext.onlinePlayerRegistry.markOnline(playerId)
